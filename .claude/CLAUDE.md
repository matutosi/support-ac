...
- 2026-09-22 (このセッション，web) **20 巻は作りかけ (ユーザーの指示で中断)**．
  記事識別子 11 件は対応表に入れ，PDF・書誌・下書き (`jstage/work/20_*`) は 11 本とも用意ずみ．
  **できたのは 4 本** (20(1):1・20(1):31・20(1):65・20(2):111．どれも DTD 妥当・エラー 0 件)．
  **独立検証まで終わったのは 20(1):1 だけ** (sonnet．指摘 0 件．`review/triage.md` あり)．
  20(1):1 の注意 14 件は，13 件が画像で載せた Table 1 の中だけで引かれている文献，
  1 件が本文にも表にも無い文献で，どちらも原文の側の事情．
  20(1):65 の注意 2 件は原文が式 (1)・(3) を番号で引いていないため．
  **独立検証・triage・zip はまだ 1 本もしていない** (20(1):65 の検証役は途中で止めた)．
  **作りかけで止めた 5 本** (20(1):17・43・55，20(2):71・83) は `body.md` が
  **途中まで直した状態**なので，再開するときは `extract_scan.py` の下書きから作り直す
  (`python $S/extract.py W/<記事識別子>.pdf --journal vegsci --out W --force`)．
  20(2):97・119 は下書きのまま手つかず．
  この巻で `build.py` を 2 件直した: 綴りを略さない「equation (2)」の式リンク，
  和文の文献をウェブ版と組にする鍵 (「ほか」で著者を省く登録版と合わなかった)．
