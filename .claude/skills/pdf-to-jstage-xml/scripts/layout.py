...
雑誌は何年かごとに組み方を変える．植生学会誌では 2026-09-20 に二分探索で次を特定した
(`jstage/pdf_fingerprint.py`)．

    31(1) → 31(2)   紙のスキャン → DTP 版        (extract_scan.py と extract.py の分かれ目)
    13(1) → 13(2)   引用文献の見出し「文献」→「引用文献」
    38(2) → 39(1)   紙 215x280mm → A4，版面が縦に 32mm 伸び，本文 9.9pt → 9.2pt
    40(1) → 40(2)   原稿種別の印字「原著論文」→「原著」
