...
6. **表**: `:::table` の枠に表を組む．まず下書きを作る．
   ...
   - **画像のまま載せる表**: 常在度表のように点線の枠・添字の範囲など XML の表で表せない組み方のものは，
     ...
     build_report に出る．
7. **図表の位置**: `:::fig`・`:::table` の枠は，初めて参照する段落の後ろへ動かす (J-STAGE の指定)．
8. **書誌**: `meta.yaml` を `pages/p001.png` と照合する．英語の氏名の姓・名の分け方，所属の対応．
9. **引用文献**: 1件1行か．`refs_web.txt` があれば件数と中身を比べる
...
- 本文の「図1」「表1」は自動で `<xref>` になる．「著者 年」の引用も自動 (著者表記のかたまりが
...
- 数式 (MathML)・電子付録・Graphical Abstract・ファンド情報は組まない．要るときは XML を足すより，
  全文 XML 作成ツールの画面で足すほうが早い．
- 本文の脚注 (`<fn>`) は扱わない．
