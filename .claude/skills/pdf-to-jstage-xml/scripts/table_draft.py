...
使い方:
    ...
    --cols      列の始まり (pt) を AI が手で与える．まばらな列 (最初の行にだけ地点名がある列など) を拾えないとき
    ...
    --next-clip 見開きの表: 次のページのこの範囲も読み，同じ高さの行を右につなぐ
                (植生学会誌 37(1) 表2 は，左ページに行の見出しと6地点，右ページに残りの15地点がある)

出力は標準出力．見出し (thead) と脚注は，tables/tableN.png を見て AI が手で直す．
