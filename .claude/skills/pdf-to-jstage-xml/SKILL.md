...
6. **表**: `:::table` の枠に表を組む．まず下書きを作る．
   ...
   - 「表層/底層」のように1件が2段になる表は `--key-col` (その列の値ごとに1行．2段の短い値は `<break/>`)．
     まばらな列 (最初の行にだけ湖名がある列) を拾えないときは `--cols` で列の始まりを与える．
...
2. Agent ツールで検証役を起動する．
   ...
   - **model: 実行時のオプション `--review` で選ばれたモデル (`opus`・`fable`・`sonnet`) にする**
     (上の「実行時のオプション」)．
     指定が無かったときは，最初に AskUserQuestion で決めてある．ここで改めて聞かない．
     選ぶときの目安は次の表 (最初に聞くときも，これを添える)．
     ...
     | sonnet | 安く速い．作った側が opus・fable のどちらでも別のモデルになる．本文の抜け・引用文献の分解・リンクの行き先のような，文字どうしの突き合わせには足りる | **未実測** (2026-09-19 時点で比べていない)．小さい字の表のセル照合や，斜体・ダッシュ・上付き下付きの判別で見落としが増えうる．見落としは誰も気づかないまま残るので，表の大きい論文には勧めない |

     - 量の目安 (実測): 37(1) (11 ページ，表2 は 21 湖 × 37 種を全セル照合) を fable で検証して
