...
- **英文要旨 (`abstract.en`) は `fetch_jstage.py` が J-STAGE の英語の記事ページの abstract から取る**．この登録の文字は紙面と
  違うことがある (上付きを `m^2` と文字で書く・空白や `℃` の違い・斜体が無い)．**21〜23 巻は現状のままでよい** (2026-09-25 ユーザ確定．
  21 巻の 4 本は紙面に合わせて直した形，22・23 巻は画面の文字のまま)．`^` の残りは `validate.py` がエラーにする．
...
- **22・23 巻の zip は手元にあるが，作業ディレクトリ (body.md・meta.yaml・PDF) は無い** (2026-09-25)．
  web のセッションがブランチ `vol22-zip`・`vol23-zip` で zip を受け渡し，MATUTOSI_DP で `jstage/work/<巻>/<開始ページ>/` と
  `jstage/work/_bundle/<巻>/vegsci.zip` に置いた (22 巻 13 個・23 巻 16 個．git の版と一致・破損なし．ブランチは削除)．
  XML (22 巻 12 本・23 巻 15 本) は DTD 妥当で，印の閉じ忘れ・article-type の誤りも無い．
  **【決定 2026-09-25・ユーザ確定】英文要旨は現状のままでよい**．22・23 巻は J-STAGE の英語の画面の abstract のまま
  (紙面との照合はしない)，21 巻の 4 本は紙面に合わせて直した形のまま．
- 23 巻で `validate.py` に入った article-type の値の点検で，**21(1):15 が `article-type="原著論文"` (和名) だった**のが見つかった．
  `research-article` に直し，21 巻のまとめた zip も作り直した (2026-09-25)．13〜22 巻の他の XML に同じ誤りは無い．
